...
CREATE TABLE quadras (
    ...
    CONSTRAINT chk_quadras_tipo CHECK (tipo IN ('FUTSAL', 'SOCIETY', 'BASQUETE', 'VOLEI', 'TENIS')),
    CONSTRAINT chk_quadras_status CHECK (status IN ('DISPONIVEL', 'INDISPONIVEL', 'MANUTENCAO')),
...
CREATE TABLE agendamentos (
    ...
    CONSTRAINT fk_agendamentos_quadra
        ...
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    CONSTRAINT chk_agendamentos_status CHECK (status IN ('PENDENTE', 'CONFIRMADO', 'CANCELADO', 'FINALIZADO')),
    CONSTRAINT chk_agendamentos_horario CHECK (hora_fim > hora_inicio)
);

-- Evita duplicidade exata. A validacao completa de sobreposicao de horarios
-- tambem e feita no DAO, pois precisa comparar intervalos parcialmente cruzados.
